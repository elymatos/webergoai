...
// Counts 4 bytes for each character just to be sure.  May take out.
...
int prlist2pyList(CTXTdeclc prolog_term V, PyObject *pList, int count)
{
	...
	{ 
		head = p2p_car(temp);
		PyObject *pyObj = NULL;
		if (convert_prObj_pyObj(CTXTc head, &pyObj) < TRUE)
			return FALSE;
...
DllExport void free_python_object_int(CTXTdeclc prolog_term prTerm) {
  PyObject *pyObj;
...
void set_python_argument(CTXTdeclc prolog_term prterm, PyObject *pArgs,int i, char *funct, int arity) {
  PyObject *pyValue;
  if(convert_prObj_pyObj(CTXTc prterm, &pyValue)  < TRUE) {
    xsb_domain_error_vargs("py_term",prterm,
			   "++Error[janus]: argument %d of %s/%d could not be translated to python"
			   "(arg 2 of py_func/py_dot/py_iter\n",i,funct,arity);
  }
  //  if (is_functor(temp) && strcmp(p2c_functor(temp),PYOBJ_C) == 0 ) {
  //      Py_INCREF(pValue);
  PyTuple_SetItem(pArgs, i-1, pyValue);
}
...
void janus_python_error() {
  PyObject *ptype, *pyValue, *ptraceback = NULL;
  PyErr_Fetch(&ptype, &pyValue, &ptraceback);
  PyObject* ptypeRepr = PyObject_Repr(ptype);
  PyObject* pyValueRepr = PyObject_Repr(pyValue);
  prolog_term error_value = p2p_new(CTXT);
  if (!convert_pyObj_prObj(pyValue, &error_value, 0) ) {
    prolog_term error_value1 = p2p_new(CTXT);
    const  char *result = PyUnicode_AsUTF8(pyValueRepr);		
    c2p_string(CTXTc (char *) result, error_value1);
    error_value = error_value1;
    }
  ...
  if (NULL != tb && NULL != tb->tb_frame ) {
    ...
    xsb_library_error_vargs(CTXTc "python_error",(char *) PyUnicode_AsUTF8(ptypeRepr),
			    error_value,"No Python backtrace found.\n");
#else
    xsb_library_error_vargs(CTXTc "python_error",(char *) PyUnicode_AsUTF8(ptypeRepr),
			      error_value,"Stack not printed for Python >= 3.11\n");
#endif
}

PyObject *get_module(Cell mod) {
...
DllExport int py_dot_int(CTXTdecl) {
  PyObject *pyModule = NULL, *pyObjIn = NULL, *pObjOut = NULL;
...
DllExport int pyObj_next_int(CTXTdecl) {
  prolog_term prTerm = extern_reg_term(1);
...
// By the Python C-API
// Tried to decref pObjIn but this didn't work. Not collecting pObjOut
DllExport int py_setattr_int(CTXTdecl) {
  PyObject *pyObjIn = NULL,*pyVal = NULL;
  prolog_term prObjIn, prMethIn, prValIn; 
  PyErr_Clear();
  prObjIn = extern_reg_term(1);
  //  Skipping several typechecks, as Python does them.
  if(is_functor(prObjIn) && strcmp(p2c_functor(prObjIn),PYOBJ_C) == 0) {
  ...
  prMethIn = (Cell) extern_reg_term(2);
  XSB_Deref(prMethIn);
  if (isstring(prMethIn)) {
    prValIn = (Cell) extern_reg_term(3);
    XSB_Deref(prValIn);
    convert_prObj_pyObj(CTXTc prValIn, &pyVal);
    int ret = PyObject_SetAttrString(pyObjIn,string_val(prMethIn),pyVal);
    if (ret == -1) 
      janus_python_error();
  }
  else {
    xsb_type_error_vargs(CTXTc "callable",prMethIn,"py_setattr/[3,4]","");
  }
  return TRUE;
}
