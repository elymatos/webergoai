...
DllExport void call_conv xsb_domain_error_vargs(CTXTdeclc char *valid_domain,Cell culprit, char *description, ...) {
  char message[MAXBUFSIZE];
  va_list args;
  prolog_term ball_to_throw;
  int isnew;
...
//static void printPlgTerm(CTXTdeclc prolog_term term) {
...
DllExport void call_conv xsb_library_error_vargs(CTXTdeclc char *functor, char *error_type,Cell error_value,
						 char *description, ...) {
  char message[MAXBUFSIZE];
  va_list args;
  prolog_term ball_to_throw;
  ...
  ball_to_throw = makecs(hreg);
  error_rec = hreg;
  hreg += 9;  // error/2 + library_error/1 + context/2
  bld_functor(error_rec, pair_psc(insert("error",2,(Psc)flags[CURRENT_MODULE],&isnew)));
  bld_cs((error_rec+1),(error_rec+3));
  bld_cs((error_rec+2),(error_rec+6));
  bld_functor((error_rec+3), pair_psc(insert("python_error",2,(Psc)flags[CURRENT_MODULE],&isnew)));
  bld_string((error_rec+4),string_find(error_type,1));
  //  bld_string((error_rec+5),string_find(error_value,1));
  bld_ref(error_rec+5,error_value);
 bld_functor(error_rec+6, pair_psc(insert("context",2,
				    (Psc)flags[CURRENT_MODULE],&isnew)));
  bld_string(error_rec+7, string_find(message,1));
  bld_copy(error_rec+8,build_xsb_backtrace(CTXT));
  //  printPlgTerm(makecs((error_rec+5)));
  ...
  if (! flags[EXCEPTION_ACTION]) {
    ...
    if (isref(op1) || isref(op2)) {
      ...
      xsb_type_error_vargs(CTXTc "integer",op2, "NULL", "Wrong type in evaluable function %s/2 (Goal: %s(%s,%s)",
			   OP,OP,str_op3.string,str_op4.string);
    } else if (isointeger(op2) && oint_val(op2) == 0) {
      xsb_domain_error_vargs(CTXTc "non-zero",op2,"divide by zero");
    } else xsb_evaluation_error(CTXTc EVALUATION_DOMAIN_ERROR,"in expression evaluation");
  }
  ...
  case ZERO_DIVIDE:
    snprintf(message,ERRMSGLEN,
	    "! %s error in %s\n! %s expected, but %lx found",
             err_msg_table[description], f, expected, (unsigned long) int_val(found));
    break;
  default:
    snprintf(message,ERRMSGLEN,
