...
\section{Using \januspy } \label{sec:using-januspy}
...
\item {\it Itertor-based}: returning answers for a Prolog goal $G$ via
  an instance of a class whose iterator backtracks through answers to
...
%keyword argument {\tt vars=N} is included in a {\tt jns.comp()} call
...
%With {\tt jns.comp()} the truth values can
%optionally be removed.  The reason {\tt jns.comp()} can support this
%is that {\tt jns.omp()} returns a list or set of {\em true} or {\em
...
\begin{description}
  ...
  \stdpymoditem{cmd(module,pred,*args)}{jns.cmd()}
%
  ...
  \stdpymoditem{apply\_once(module,pred,*args,**kwargs)}{jns.apply\_once()}
%
...
%     \ei
  ...
  \stdpymoditem{query\_once(query\_string,**kwargs)}{query\_once()}
  %
...
\stdpymoditem{apply(module,pred,*args)}{apply()}
%
...
\stdpymoditem{query(query\_string,**kwargs)}{query()}
%
...
\stdpymoditem{close\_query()}{close\_prolog\_goal()} Closes a
...
\begin{description}
  ...
  \repeatstdpymoditem{consult(File)}{consult()}
  \stdpymoditem{ensure\_loaded(File)}{ensure\_loaded()}
%
